...
const fetchMarks = async () => {
  try {
    loading.value = true
    const response = await fetch(`${NEODB_API_BASE}/user/${NEODB_USERNAME}/marks?category=book&page=1`, {
...
<template>
  <div class="neodb-books">
    ...
    <div class="text-center mt-6">
      <a
        :href="`https://neodb.social/user/${NEODB_USERNAME}/marks?category=book`"
        target="_blank"
        rel="noopener noreferrer"
